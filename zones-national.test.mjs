...
try {
 ...
 await check('LEGACY BACKFILL: omitted national fields default CT and market',async()=>{const legacy=id();await upsertZoneCore(actor,{id:legacy,name:'Legacy',lat:41,lng:-73});const z=(await getDispatchZonesForOwnerCore(actor)).zones.find(x=>x.id===legacy);assert.equal(z.state,'CT');assert.equal(z.zoneType,'market')});
 await check('REGRESSION: legacy upsert remains valid',async()=>{const r=await upsertZoneCore(actor,{id:id(),name:'Legacy regression',lat:41,lng:-73});assert.equal(r.ok,true)});
