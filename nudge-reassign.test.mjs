...
try {
 ...
 const run=async(id,drivers,opts={})=>{const f=mockFetch(drivers,opts).f; globalThis.fetch=(url,init)=>(String(url).includes("app.towbook.com")||String(url).includes("api.tomtom.com"))?f(url,init):realFetch(url,init); await processAssignmentNudges(ORG,now);};
 // a
