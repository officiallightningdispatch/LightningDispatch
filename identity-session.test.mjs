...
try {
  ...
  const applied=await q`SELECT COUNT(*)::int n FROM schema_migrations`; check('f migration state ends at 50',Number((await q`SELECT MAX(version)::int n FROM schema_migrations`)[0].n)===50);
} finally {
