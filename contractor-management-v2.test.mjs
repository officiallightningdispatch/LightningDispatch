...
// listContractorsCore lives in the contractor-management core (same module the
// v1 suite imports it from), not in contractor-admin-core.
...
async function setup() {
  ...
  for (const [org, owner, driver] of [[ORG, OWNER, DRIVER], [OTHER_ORG, OTHER_OWNER, null]]) {
    ...
    await q`INSERT INTO users(id, name, email, password_hash) VALUES(${owner}, 'QA V2 Owner', ${owner + "@lightning.test"}, 'x')`;
    await q`INSERT INTO organization_memberships(org_id, user_id, role) VALUES(${org}, ${owner}, 'owner')`;
    if (driver) {
      await q`INSERT INTO users(id, name, email, password_hash, towbook_driver_id) VALUES(${driver}, 'QA V2 Driver', ${`qa-contractor-v2-driver-${TAG}@lightning.test`}, 'x', ${'v2-1001-' + TAG})`;
...
{
  // Section 2 left an owner-overridden schedule; the v2 rule blocks driver
  // edits while owner_override is set (asserted below). Reset the row so the
  // driver round-trip starts from the declared-availability state.
  await q`DELETE FROM contractor_schedules WHERE org_id=${ORG} AND user_id=${DRIVER}`;
...
/* ================= 5) DocCompareSheet data (license + selfie) ================= */
{
  ...
  check("compare: non-facial type has no selfie rail (selfieStatus missing, rail gated on requiresFacialVerification)", insurance && insurance.requiresFacialVerification === false && insurance.selfieStatus === "missing", JSON.stringify(insurance));
...
// users have no org FK (same rule the v1 suite notes): collect member ids
// BEFORE org deletion, delete members explicitly, then sweep any orphaned
// fixture users from crashed runs (their orgs are already gone).
// Org sweep uses the broad v1-style 'qa%contractor-v2%' pattern so orgs left by
// earlier runs (older name format 'qa contractor v2') are removed too.
const v2MemberIds = await q`SELECT DISTINCT m.user_id FROM organization_memberships m JOIN organizations o ON o.id=m.org_id WHERE o.name LIKE 'qa%contractor-v2%'`;
for (const org of await q`SELECT id, name FROM organizations WHERE name LIKE 'qa%contractor-v2%'`) {
  await assertQaOrg(org.id, org.name);
  await q`DELETE FROM organizations WHERE id=${org.id}`;
}
for (const m of v2MemberIds) await q`DELETE FROM users WHERE id=${m.user_id}`.catch(() => {});
// View-toggle self-link (owner.linked_driver_user_id → driver user) is a plain
// FK that blocks bulk user deletes — clear it before sweeping leftovers.
await q`UPDATE users SET linked_driver_user_id=NULL WHERE email LIKE 'qa-contractor-v2-%@lightning.test'`.catch(() => {});
await q`DELETE FROM users WHERE email LIKE 'qa-contractor-v2-%@lightning.test'`.catch(() => {});
const final = await q`
  SELECT
    (SELECT COUNT(*)::int FROM organizations WHERE name LIKE 'qa%contractor-v2%') AS orgs,
    (SELECT COUNT(*)::int FROM users WHERE email LIKE ${`qa-contractor-v2-%@lightning.test`}) AS users,
