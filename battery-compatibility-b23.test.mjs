...
const { decodeVin } = await import("./src/data/battery-sales-core.ts");
const VIN = "1HGCM82633A004352";
const nhtsa = (_url, _opts) => Promise.resolve(new Response(JSON.stringify({Results:[{Make:"CHEV",Model:"F-150",ModelYear:"2022",Trim:"",EngineModel:""}]}), {status:200,headers:{"content-type":"application/json"}}));
check("NHTSA aliases canonicalize", (await decodeVin(VIN,nhtsa)).ok);
check("NHTSA failure graceful", (await decodeVin(VIN, async()=>new Response("",{status:503}))).ok === false);
check("VIN lookup fail closed on empty table", (await core.lookupBatteryCompatibilityFromVinCore({orgId:"qa-no-db",role:"owner"},{vin:VIN},nhtsa)).outcome === "review");
check("VIN DTO has no raw VIN", JSON.stringify(await core.lookupBatteryCompatibilityFromVinCore({orgId:"qa-no-db",role:"owner"},{vin:VIN},nhtsa)).includes(VIN) === false);
...
check("VIN endpoint is review-only and does not expose raw VIN",source.includes("decode_failed")&&!source.includes("rawVin"));
