...
export function AppShell({
  ...
  return <div className={`min-h-dvh min-w-0 overflow-x-clip bg-canvas text-ink-900 ${meta.mobileBottomPad ? "pb-20" : ""}`}>
    <header className="border-b border-ink-100 bg-surface"><div className={`mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 sm:px-6 ${portal === "driver" ? "flex-wrap" : ""}`} style={{ paddingTop: "max(0.75rem, env(safe-area-inset-top))" }}><Link to={links[0].to as any} className="flex min-w-0 items-center gap-3">{portal === "driver" ? <img src="/brand-roadside.jpg" alt="Lightning Roadside Assistants" className="h-9 w-auto shrink-0 object-contain" /> : <><img src="/brand-dispatch.jpg" alt="Lightning Dispatch" className="size-9 shrink-0 rounded-xl object-cover" /><strong className="min-w-0 truncate text-sm font-bold">Lightning Dispatch OS</strong></>}</Link><span className="hidden text-xs text-ink-400 sm:block">{meta.appLabel}</span>{actions || portal === "driver" ? <div className="ml-auto flex items-center gap-2">{identity.staffWithDriver && portal === "owner" ? <DriverViewPill /> : null}{actions}{portal === "driver" && (
  ...
  <Link to="/driver/profile" aria-label="Profile" className="flex items-center">
    ...
    <nav className="fixed inset-x-0 bottom-0 z-20 flex border-t border-ink-100 bg-surface/95 p-1 backdrop-blur md:hidden" style={{ paddingBottom: "calc(0.25rem + env(safe-area-inset-bottom))" }} aria-label="Portal navigation">{mobile.map(l => <Link key={l.to} to={l.to as any} className={`flex min-h-11 min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-lg px-0.5 text-[10px] font-semibold ${isActive(l.to) ? "text-brand-600" : "text-ink-500"}`}><l.icon className="size-4 shrink-0" /><span className="w-full truncate text-center">{l.label}</span></Link>)}{more && more.length > 0 ? <button type="button" onClick={() => setMoreOpen(true)} aria-haspopup="dialog" aria-expanded={moreOpen} aria-label="More options" className={`flex min-h-11 min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-lg px-0.5 text-[10px] font-semibold ${more.some(l => isActive(l.to)) ? "text-brand-600" : "text-ink-500"}`}>
