...
export function AppShell({
  ...
  return <div className={`min-h-dvh min-w-0 overflow-x-clip bg-canvas text-ink-900 ${meta.mobileBottomPad ? "pb-20" : ""}`}>
    <header className="border-b border-ink-100 bg-surface"><div className={`mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 sm:px-6 ${portal === "driver" ? "flex-wrap" : ""}`} style={{ paddingTop: "max(0.75rem, env(safe-area-inset-top))" }}><Link to={links[0].to as any} className="flex min-w-0 items-center gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-brand-500"><Zap className="size-5 text-white" fill="currentColor" strokeWidth={0} /></span><strong className="min-w-0 truncate text-sm font-bold">Lightning Dispatch OS</strong></Link><span className="hidden text-xs text-ink-400 sm:block">{meta.appLabel}</span>{actions || portal === "driver" ? <div className="ml-auto flex items-center gap-2">{identity.staffWithDriver && portal === "owner" ? <DriverViewPill /> : null}{actions}{portal === "driver" && (
