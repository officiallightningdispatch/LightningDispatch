...
export const setJobStatus=createServerFn({method:"POST"}).validator(passthrough).handler(async({data})=>{
  ...
  try{
    ...
    if(target === "completed" || target === "cancelled") { try { const sale=await q`SELECT id,product_id FROM battery_sales WHERE org_id=${u.orgId} AND job_id=${jobId} AND status IN ('approved','paid') LIMIT 1`; if(sale.length && sale[0].product_id && target !== "completed"){ const {releaseBatteryInventory}=await import("./battery-lifecycle-core"); await releaseBatteryInventory(q,{orgId:u.orgId,saleId:String(sale[0].id),productId:String(sale[0].product_id),jobId:jobId,actorUserId:u.id}); } } catch {} }
