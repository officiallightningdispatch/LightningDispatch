...
async function pushJobStatus(orgId: string, jobId: string, actor: { id: string; role: AuthUser["role"] }): Promise<void> {
  try {
    const { pushJobStatusToTowbook } = await import("./status-push-core");
    await pushJobStatusToTowbook({ orgId, jobId, actor: { id: actor.id, role: actor.role } });
  } catch { /* push problems escalate internally — never fail the command */ }
}
