...
export const authStatus=createServerFn({method:"GET"}).handler(async()=>{const s=await server();if(!process.env.DATABASE_URL)return {mode:"unavailable" as const,user:null,error:"Database is not configured."};const user=await s.currentUser();const q=(await import("~/db")).sql();const count=await q`SELECT count(*)::int AS count FROM users WHERE id IN (SELECT user_id FROM organization_memberships WHERE role='owner')`;return {mode:"database" as const,needsOwner:Number((count[0] as Record<string,unknown>).count)===0,user};});
