...
export async function zoneContainingPointCore(a:ZoneActor, lat:number, lng:number, zip?:string){
 const q=qdb(); const rows=await q`SELECT id,org_id,name,state,market,zone_type,zip_codes,parent_zone_id,lat,lng,radius_miles,tz,active,sort_order,polygon_geojson,capacity,market_id,demand_level,status,is_reserved,unlock_jobs_required,color FROM dispatch_zones WHERE org_id=${a.orgId} AND active=TRUE ORDER BY sort_order,name`;
 const {zoneContainingPoint}=await import('../lib/zone-containment'); const match:any=zoneContainingPoint(rows as any,lat,lng,zip);
...
export async function deleteZoneCore(a:ZoneActor, zoneId:string){
 if(!can(a)) return {ok:false as const,message:'Owner access required.'};
 const q=qdb();
 const rows=await q`SELECT id,name,zone_type FROM dispatch_zones WHERE id=${zoneId} AND org_id=${a.orgId} AND active=TRUE LIMIT 1`;
 if(!rows.length) return {ok:false as const,message:'Zone not found.'};
 const zone=rows[0];
 if(String(zone.zone_type)==='coverage') return {ok:false as const,message:'Structural coverage zones cannot be deleted.'};
 const children=await q`SELECT 1 FROM dispatch_zones WHERE parent_zone_id=${zoneId} AND org_id=${a.orgId} AND active=TRUE LIMIT 1`;
 if(children.length) return {ok:false as const,message:'This zone has child zones and cannot be deleted.'};
 const online=await q`SELECT 1 FROM driver_availability_log WHERE org_id=${a.orgId} AND zone_id=${zoneId} AND session_started_at IS NOT NULL AND heartbeat_at > NOW() - INTERVAL '90 seconds' LIMIT 1`;
 if(online.length) return {ok:false as const,message:'This zone has an online driver and cannot be deleted.'};
 await q`UPDATE dispatch_zones SET active=false, updated_at=NOW() WHERE id=${zoneId} AND org_id=${a.orgId}`;
 await q`INSERT INTO audit_log(id,org_id,actor_user_id,actor_role,action,entity_type,entity_id,detail) VALUES(${randomUUID()},${a.orgId},${a.id},${a.role},'zone_deleted','zone',${zoneId},${JSON.stringify({name:String(zone.name)})}::jsonb)`;
 return {ok:true as const};
}
export async function upsertZoneCore(a:ZoneActor,v:any){if(!can(a))return {ok:false as const,message:'Owner access required.'};const q=qdb();const id=String(v.id??randomUUID());if(v.state==null||String(v.state).trim()==='')throw new Error('State is required.');const state=String(v.state).toUpperCase();await ensureStateNode(a.orgId,state);const parentId=v.parentZoneId==null?null:String(v.parentZoneId);if(parentId===id)return {ok:false as const,message:'A zone cannot be its own parent.'};if(parentId){const p=await q`SELECT id,state FROM dispatch_zones WHERE id=${parentId} AND org_id=${a.orgId} LIMIT 1`;if(!p.length)return {ok:false as const,message:'Parent zone not found in this organization.'};if(p[0].state&&v.state&&String(p[0].state)!=='US'&&String(p[0].state)!==String(v.state).toUpperCase())return {ok:false as const,message:'Parent and child must share a state.'};let cursor=parentId;for(let i=0;i<32;i++){const r=await q`SELECT parent_zone_id FROM dispatch_zones WHERE id=${cursor} AND org_id=${a.orgId} LIMIT 1`;if(!r.length)break;cursor=r[0].parent_zone_id?String(r[0].parent_zone_id):'';if(cursor===id)return {ok:false as const,message:'Zone hierarchy cycle detected.'};if(!cursor)break;} }const zoneState=state;const zoneType=v.zoneType??'market';const demandLevel=clampDemandLevel(v.demandLevel);const status=validateZoneStatus(v.status??'available');const isReserved=Boolean(v.isReserved??false);const unlockJobsRequired=validateUnlockJobs(v.unlockJobsRequired??30);const color=validateHexColor(v.color);const zips=Array.isArray(v.zipCodes)?v.zipCodes.map(String):[];const defaultActive=zoneType==='coverage'||zoneType==='US'||zoneType==='state';const active=v.active??defaultActive;await q`INSERT INTO dispatch_zones(id,org_id,name,state,market,zone_type,zip_codes,parent_zone_id,lat,lng,radius_miles,tz,active,sort_order,polygon_geojson,capacity,market_id,demand_level,status,is_reserved,unlock_jobs_required,color,updated_at) VALUES(${id},${a.orgId},${v.name},${zoneState},${v.market??''},${zoneType},${zips},${parentId},${v.lat},${v.lng},${v.radiusMiles??20},${v.tz??'America/New_York'},${active},${v.sortOrder??0},${v.polygonGeojson??null},${v.capacity??null},${v.marketId??null},${demandLevel},${status},${isReserved},${unlockJobsRequired},${color},NOW()) ON CONFLICT(id) DO UPDATE SET name=${v.name},state=${zoneState},market=${v.market??''},zone_type=${zoneType},zip_codes=${zips},parent_zone_id=${parentId},lat=${v.lat},lng=${v.lng},radius_miles=${v.radiusMiles??20},tz=${v.tz??'America/New_York'},active=${active},sort_order=${v.sortOrder??0},polygon_geojson=${v.polygonGeojson??null},capacity=${v.capacity??null},market_id=${v.marketId??null},demand_level=${demandLevel},status=${status},is_reserved=${isReserved},unlock_jobs_required=${unlockJobsRequired},color=${color},updated_at=NOW() WHERE dispatch_zones.org_id=${a.orgId}`;return {ok:true as const,id};}
