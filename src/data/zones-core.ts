...
export async function upsertZoneCore(a:ZoneActor,v:any){if(!can(a))return {ok:false as const,message:'Owner access required.'};const q=qdb();const id=String(v.id??randomUUID());const state=v.state?String(v.state).toUpperCase(): 'CT';const ensured=await ensureStateNode(a.orgId,state);const parentId=v.parentZoneId==null?ensured.stateId:String(v.parentZoneId);if(parentId===id)return {ok:false as const,message:'A zone cannot be its own parent.'};if(parentId){const p=await q`SELECT id,state FROM dispatch_zones WHERE id=${parentId} AND org_id=${a.orgId} LIMIT 1`;if(!p.length)return {ok:false as const,message:'Parent zone not found in this organization.'};if(p[0].state&&v.state&&String(p[0].state)!=='US'&&String(p[0].state)!==String(v.state).toUpperCase())return {ok:false as const,message:'Parent and child must share a state.'};let cursor=parentId;for(let i=0;i<32;i++){const r=await q`SELECT parent_zone_id FROM dispatch_zones WHERE id=${cursor} AND org_id=${a.orgId} LIMIT 1`;if(!r.length)break;cursor=r[0].parent_zone_id?String(r[0].parent_zone_id):'';if(cursor===id)return {ok:false as const,message:'Zone hierarchy cycle detected.'};if(!cursor)break;} }const zoneState=state;const zoneType=v.zoneType??'market';const zips=Array.isArray(v.zipCodes)?v.zipCodes.map(String):[];await q`INSERT INTO dispatch_zones(id,org_id,name,state,market,zone_type,zip_codes,parent_zone_id,lat,lng,radius_miles,tz,active,sort_order,updated_at) VALUES(${id},${a.orgId},${v.name},${zoneState},${v.market??''},${zoneType},${zips},${parentId},${v.lat},${v.lng},${v.radiusMiles??20},${v.tz??'America/New_York'},${v.active??true},${v.sortOrder??0},NOW()) ON CONFLICT(id) DO UPDATE SET name=${v.name},state=${zoneState},market=${v.market??''},zone_type=${zoneType},zip_codes=${zips},parent_zone_id=${parentId},lat=${v.lat},lng=${v.lng},radius_miles=${v.radiusMiles??20},tz=${v.tz??'America/New_York'},active=${v.active??true},sort_order=${v.sortOrder??0},updated_at=NOW() WHERE dispatch_zones.org_id=${a.orgId}`;return {ok:true as const,id};}
