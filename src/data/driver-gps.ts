/**
 * GPS tracking + geofence auto-arrive (owner-directed 2026-08-11, milestone #3
 * of the driver-app plan). Works with driver-auth.ts:
 *
 *  - pingDriverLocation: while a driver has a job in en_route (or arrived), the
 *    driver portal captures browser geolocation every ~20s and pings it here.
 *    Each ping (a) is stored in driver_locations (append-light, pruned to 24h
 *    on write), (b) best-effort pushes the SAME fix to Towbook via
 *    POST /api/user/checkin with the DRIVER's own session so Towbook has live
 *    GPS (a failure never breaks the ping loop), and (c) runs the geofence
 *    check.
 *
 *  - Geofence auto-arrive: when a driver in en_route comes within
 *    org_settings.geofence_radius_meters (default 150) of their assigned job's
 *    pickup waypoint (haversine), the platform auto-transitions the job to
 *    arrived — (a) on-platform (dispatch_jobs + status_events + audit_log),
 *    (b) Towbook PUT /api/calls/{id} {id, status:{id:4}} via the DRIVER's
 *    session so the change is attributed to the real driver, then (c) VERIFIES
 *    the Towbook status actually changed (GET the call) and records the
 *    outcome. A failed Towbook write is recorded + escalated into the decision
 *    ledger (ops "Needs attention" banner) — never silently swallowed. The 30s
 *    sync reconciles any divergence (Towbook is the system of record), so a
 *    failed write self-heals on the next ping after sync.
 *
 *  - Photos gate (owner spec): auto-arrive is also gated on 4 pre-arrival
 *    photos + a vehicle-match confirmation once org_settings.photos_required
 *    is true. The photo workflow is milestone #4; the gate READS the
 *    pre_arrival_photos table (created in migration v11) and the flag defaults
 *    to FALSE, so geofence auto-arrive works today and #4 flips the flag on
 *    without touching this logic (see photosCompleteForJob TODO).
 *
 * No-fire rails: never auto-arrive for a job that is not en_route, for a ping
 * without a valid GPS fix (0,0 = geolocation denied), or when the driver is
 * not the assigned contractor (assets[].driver.id on the raw call, or the
 * legacy assigned_contractor_id).
 *
 * Client-safety (build-critical): this module is imported by client routes, so
 * every Node-only dependency (db, auth-server) is loaded via dynamic import
 * inside the server-fn handlers — never statically. Towbook HTTP goes through
 * driver-auth.ts's tbFetch/isExpired/loadDriverSession/driverCheckin (a
 * client-safe module). Every Towbook-facing function takes an injectable
 * fetchImpl for hermetic tests — no real Towbook calls in tests.
 */
...
export const getGeofenceSettingsFn = createServerFn({ method: "GET" }).handler(async () => {
