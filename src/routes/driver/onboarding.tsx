...
import { isNative, requestLocation, requestMotionPermission } from "~/lib/native-capabilities";
import { ensureNativePushRegistration } from "~/lib/push-client";
...
function Onboarding() {
  ...
  const [motionPermission,setMotionPermission]=useState<PermissionState>("needed");
  ...
  const askPermissions=async()=>{
    if (isNative()) {
      ...
      try {
        const push = await ensureNativePushRegistration();
        notificationsOk = push.ok;
      } catch { notificationsOk = false; }
      try { locationOk = await requestLocation(); } catch { locationOk = false; }
      setPermissions(notificationsOk && locationOk ? "done" : "needed");
      return;
    }
  ...
  const askMotionPermission=async()=>{
    ...
    setMotionPermission(granted ? "done" : "needed");
  };
  ...
  const complete=[Boolean(application?.agreementsAcceptedAt),compliance!=null&&compliance.required>0&&compliance.approved>=compliance.required,permissions==="done",motionPermission==="done"].filter(Boolean).length;
  ...
  const items=[
    ...
    {title:"Notifications and location",detail:permissions==="done"?"Permissions enabled":"Required for offers and accurate dispatch location",action:"permissions",done:permissions==="done",icon:Bell},
    {title:"Motion & Fitness",detail:motionPermission==="done"?"Motion & Fitness enabled":motionPermission==="unsupported"?"Open Lightning Dispatch on iPhone to enable Motion & Fitness":"Required on iPhone for movement-aware dispatch location",action:"motion",done:motionPermission==="done",icon:ShieldCheck},
  ...
  return <AppShell portal="driver" title="Onboarding" description="Finish your checklist and track owner approval."><div className="mx-auto max-w-2xl space-y-4">
    ...
    <div className="space-y-2">{items.map((item,i)=>{const Icon=item.icon;const body=<><span className={`grid size-10 shrink-0 place-items-center rounded-xl ${item.done?"bg-success-50 text-success-700":"bg-ink-100 text-ink-500"}`}>{item.done?<CheckCircle2 className="size-5"/>:<Icon className="size-5"/>}</span><span className="min-w-0 flex-1"><span className="block text-sm font-bold text-ink-800">{item.title}</span><span className="block text-xs leading-5 text-ink-500">{item.detail}</span></span>{item.action==="permissions"?<Button type="button" size="sm" variant="secondary" onClick={()=>void askPermissions()}>Enable</Button>:item.action==="motion"?<Button type="button" size="sm" variant="secondary" onClick={()=>void askMotionPermission()}>Enable</Button>:<ChevronRight className="size-4 shrink-0 text-ink-400"/>}</>;return item.action?<Card key={i} className="flex items-center gap-3 p-4">{body}</Card>:<Link key={i} to={item.to as any} className="flex items-center gap-3 rounded-2xl bg-surface p-4 ring-1 ring-ink-100 hover:bg-hover">{body}</Link>})}</div>
