...
- Build: `bun run build:cap`, then `bunx cap sync`, `bunx cap open ios` / `bunx cap open android`.
  - `build:cap` runs `vite.capacitor.config.ts`, a static (SPA) build that emits
    `dist/capacitor/client/index.html` + hashed assets — the `index.html` the
    native shell requires. The SSR web build (`bun run build`) intentionally
    emits no `index.html` (SSR owns `/`), so the native shell reads the dedicated
    `dist/capacitor/client` output instead of `dist/client`.
