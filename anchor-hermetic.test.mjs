...
check("A: placeholder Agero offer anchors to resolved TX pickup", () => {
  ...
  assert.match(source, /placeholder coords suppressed/);
...
// Scenario B: non-placeholder coordinates remain the existing search origin.
check("B: non-placeholder offer retains offer-coordinate anchor", () => {
  assert.match(source, /!isAgeroPlaceholderCoords\(lat, lng\)/);
  assert.match(source, /lookup anchored at offer coordinates \(\$\{lat\},\$\{lng\}\)/);
});
...
check("C: unresolved placeholder is retained fail-closed", () => {
  assert.match(source, /lookup retained known Agero placeholder/);
  assert.match(source, /real pickup unresolved/);
  assert.match(source, /return \{ lat, lng, note: `lookup retained known Agero placeholder/);
});
